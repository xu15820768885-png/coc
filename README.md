...
部署在 NAS 上的村庄升级倒计时服务。提交村庄 JSON 后，服务会列出所有进行中的升级及其北京时间，并在建筑升级完成时通过企业微信自建应用发送消息。

调度器不会固定轮询村庄数据。导入 JSON 后会直接计算升级完成时刻并休眠，只在建筑实际完成时发送通知；收到新 JSON 时会自动唤醒并重新安排。只有企业微信发送失败时，才会按默认 30 秒间隔重试。
...
5. Docker 解密消息并导入后，会回复建筑名称、等级、剩余时间和预计完成的北京时间；随后在建筑升级完成时自动通知。
